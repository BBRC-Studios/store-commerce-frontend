  ...
  construct(builder) {
    ...
    if (envNode.isTextureNode && envNode.value.isCubeTexture !== true) {
      ...
      if (cacheEnvNode === undefined) {
        ...
        const cubeRTT = builder
          .getCubeRenderTarget(512)
    ...
    const radiance = context(envNode, createRadianceContext(roughness, transformedNormalView)).mul(
      intensity
    );
    const irradiance = context(envNode, createIrradianceContext(transformedNormalWorld))
      .mul(Math.PI)
      .mul(intensity);
