...
class WebGLNodeBuilder extends NodeBuilder {
  ...
  getUniforms(shaderStage) {
    ...
    for (const uniform of uniforms) {
      if (/^(modelViewMatrix|projectionMatrix)$/.test(uniform.name)) continue;
  ...
  getAttributes(shaderStage) {
    ...
    if (shaderStage === 'vertex') {
      ...
      for (const attribute of attributes) {
        ...
        if (/^(position|normal|uv\d?)$/.test(attribute.name)) continue;
  ...
  getVaryings(shaderStage) {
    ...
    if (shaderStage === 'vertex') {
      for (const varying of varyings) {
        snippet += `${varying.needsInterpolation ? 'varying' : '/*varying*/'} ${varying.type} ${
          varying.name
        }; `;
      }
