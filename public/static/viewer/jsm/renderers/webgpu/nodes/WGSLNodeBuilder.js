  ...
  getUniformFromNode(node, type, shaderStage, name = null) {
    ...
    if (nodeData.uniformGPU === undefined) {
      ...
      if (type === 'texture' || type === 'cubeTexture') {
        ...
        const index =
          lastBinding && lastBinding.isUniformsGroup ? bindings.length - 1 : bindings.length;
      ...
      if (uniform.type === 'texture' || uniform.type === 'cubeTexture') {
        if (shaderStage === 'fragment') {
          ...
          if (texture.isDepthTexture === true && texture.compareFunction !== null) {
            ...
            bindingSnippets.push(
              `@binding( ${index++} ) @group( 0 ) var ${uniform.name}_sampler : sampler;`
            );
          }
        }
        ...
        bindingSnippets.push(
          `@binding( ${index++} ) @group( 0 ) var ${uniform.name} : ${textureType};`
        );
